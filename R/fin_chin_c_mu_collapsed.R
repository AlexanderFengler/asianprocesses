#' Title Chinese Restaurant Process Finite Dimensions (known variance)
#' @return
#' @export
#' @examples
fin_chin_c_mu_collapsed = function(n_data = 90, mu_s = c(-5,0,5), p_mu_s = c(0.6,0.2,0.2), sd_s = 0.5, sd_0 = 10, mu_0 = 0, alpha = 90/3, n_samples = 2000){
  ...
  mu_samples = matrix(rep(0,mu_s_len*n_samples*n_data),nrow = n_samples*n_data, ncol = mu_s_len)
  mu_cnt = 1

  while (cnt <= n_samples-1){
    ...
    for (i in x_seq){ # we go through each data point i
      ...
      mu_samples[mu_cnt,] = ((mu_0/sd_0^2) + ((c_temp[,-i] %*% x_s[-i]) / sd_s^2))  / ((1/sd_0^2) + (temp_category_count/sd_s^2))
      P_x_I_c = dnorm(x_s[i],
                      mean = ((mu_0/sd_0^2) + ((c_temp[,-i] %*% x_s[-i]) / sd_s^2))  / ((1/sd_0^2) + (temp_category_count/sd_s^2)),
                      sd = sqrt(1/((1/sd_0^2) + (temp_category_count/sd_s^2)) + sd_s^2)) # calculate p(X | c_i = k)
    ...
    loglik_post[cnt] = sum(dnorm(x_s,mean = t(((mu_0/sd_0^2) + (c_temp %*% x_s) / sd_s^2)  / ((1/sd_0^2) + (apply(c_temp, sum, MARGIN = 1)/sd_s^2))) %*% c_temp,
                                 sd = sqrt(1/((1/sd_0^2) + (apply(c_temp, sum, MARGIN = 1)/sd_s^2)) + sd_s^2) %*% c_temp,
                                 log = T)) +
  ...
  # adjust sample category names to match data
  c_samples_means = apply(mu_samples,mean,MARGIN = 2)
  cluster_order = c(apply(abs(matrix(rep(mu_s,each = length(mu_s)),nrow = length(mu_s), byrow = TRUE) - matrix(rep(apply(mu_samples,mean,MARGIN = 2),times = length(mu_s)), nrow = length(mu_s), byrow = TRUE)), which.min, MARGIN = 2),mu_s_len + 1, mu_s_len + 2)
